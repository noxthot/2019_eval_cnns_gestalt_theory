...
path = os.path.join(f'generated_images{imageSize}x{imageSize}', 'kanizsa_triangle_all_non_pacman') # place where generated images are stored
##
...
for i in range(2):
    ...
    while imgNr < nrOfImagesPerClass:
        ...
        while True:
            posRand1 = (np.array([random.uniform(0, 1) for x in range(2)]) * (imageSize - 2.5 * size) + 1.25 * np.array([size, size])).astype(int)
            posRand2 = (np.array([random.uniform(0, 1) for x in range(2)]) * (imageSize - 2.5 * size) + 1.25 * np.array([size, size])).astype(int)
            posRand3 = (np.array([random.uniform(0, 1) for x in range(2)]) * (imageSize - 2.5 * size) + 1.25 * np.array([size, size])).astype(int)
